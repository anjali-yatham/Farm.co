...
// Cron job: runs every minute (gated by env flag and API key)
const ENABLE_SMS_REMINDERS = process.env.ENABLE_SMS_REMINDERS === 'true';
const HAS_FAST2SMS_KEY = Boolean(process.env.FAST2SMS_API_KEY);

if (ENABLE_SMS_REMINDERS && HAS_FAST2SMS_KEY) {
  cron.schedule('* * * * *', async () => {
    const now = new Date();
    ...
    for (const reminder of dueReminders) {
      ...
      if (user && user.phone) {
        ...
        try {
          const result = await sendReminderSMS(user.phone, user.name, reminder.message, reminder.date);
          ...
          if (result && result.success === true) {
            ...
            console.log(`[CRON] SMS sending skipped for reminder ID: ${reminder._id} (${result.reason || 'unknown reason'})`);
          } else {
            console.warn(`[CRON] SMS not sent for reminder ID: ${reminder._id}; will retry on next run`);
          }
        } catch (err) {
  ...
  console.log('[CRON] Reminder scheduler enabled. Set ENABLE_SMS_REMINDERS=false to disable.');
} else {
  console.log('[CRON] Reminder scheduler disabled. Reason:', {
